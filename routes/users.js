...
router.get("/:email",(req,res)=>{
  ...
  // Filter the users array to fins users whose email matches the extracted email parameter
  let filtered_users = users.filter((user) => user.email === email);
  // Send the filtered_users array as the response to the client
  res.send(filtered_users);
});

router.get("/lasName/:lastName", (req, res) => {
...
// Function to convert a date string in the format "dd-mm-yyyy" to a Data object
...
router.put("/:email", (req, res) => {
  // Extract email parameter and find users with marching email
  const email = req.params.email;
  let filtered_users = users.filter((user) => user.email === email);
  ...
  if (filtered_users.length > 0) {
    ...
    */
    ...
    // Extract and update firstName is provided
